...
void main(void)
{
	vec2 offset = vec2(0.0,u_time*0.1);
	float n = 30.0;
	...
	vec3 stars = vec3(0.0, 0.0, 0.0);//clouds(p.xy+offset*n,5.0);
	vec3 color = getcolor(p.xy+offset*n,11.0)*0.2;
	color = BlendMultiply(stars, color);
	stars = genstars(3.0, 70.0, 1.0, p.xy + offset * n);
	color = BlendScreen(color, stars);
	n=20.0;
	stars = genstars(2.0, 70.0,1.0,p.xy+offset*n);
	n=10.0;
	color = BlendScreen(color,stars);
